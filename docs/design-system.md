...
## Public identity
The favicon uses the authored `favicon.ico` asset. Do not replace it with an autogenerated alternate unless that change is intentional. The Open Graph card follows the restrained visual language: dark field, missing first position, chromatic signal line, name and concise positioning.
